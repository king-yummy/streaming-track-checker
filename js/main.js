...
async function loadTodoListData() {
  try {
    ...
    sessionStorage.setItem("todoData", JSON.stringify(allTodoData)); // 세션 스토리지에 저장
    renderTodoList(allTodoData);
  } catch (error) {
...
async function loadStreamingList() {
  try {
    ...
    sessionStorage.setItem("streamingSchedule", JSON.stringify(schedule)); // 세션 스토리지에 저장
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (sessionStorage.getItem("isInitialized")) {
    ...
    async function loadAllData() {
      ...
      sessionStorage.setItem("isInitialized", "true"); // 초기화 완료 플래그 설정
      loadingScreen.style.display = "none";
      mainContent.classList.remove("invisible");
    }
    loadAllData();
